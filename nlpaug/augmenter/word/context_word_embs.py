...
def init_context_word_embs_model(
    model_path,
    device,
    force_reload=False,
    temperature=1.0,
...
class ContextualWordEmbsAug(WordAugmenter):
    ...
    def split_text(self, data):
        ...
        if (
            ...
            return (
                ...
                tokens,
                None,
            )  # Head text, tail text, head token, tail token
    ...
    def substitute(self, data):
        ...
        for i in range(max_aug_size):
            ...
            for original_token, aug_input_pos, output, masked_text in zip(
                original_tokens, aug_input_poses, outputs, masked_texts
            ):
                split_result = split_results[aug_input_pos]
                head_doc = split_result[5]
                aug_idx = split_result[6][i]  # augment position in text
    ...
    def get_model(
        ...
        return init_context_word_embs_model(
            model_path,
            device,
            force_reload,
            temperature,
